...
public class Test {
    ...
    public static StringBuilder recordString=new StringBuilder();//登录后读取文本中的记录，然后和 recordString 拼接
...
/**********************并写入 list**********************/
        ...
        String temp = "";
        try {
            System.out.println("开始写入 list");
            while ((temp = bfr.readLine()) != null) {//不知为何读取出的字符串中最前面会出现 Null
            ...
            bfr.close();
            fw.close();
            System.out.println("用户 list:"+usersList);
        } catch (Exception e) {
